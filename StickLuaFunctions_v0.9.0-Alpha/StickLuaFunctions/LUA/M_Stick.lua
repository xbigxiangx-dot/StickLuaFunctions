...
local st_roundTimer = {}
function ST_Hook_AttackIndex(sourceSpriteId, targetSpriteId, attackIndex)
	...
	if not st_roundCounter[sourceSpriteId] then
		st_roundCounter[sourceSpriteId] = 0
		st_roundTimer[sourceSpriteId] = Infinity_GetGameTicks()
	elseif attackIndex == 1 then
		st_roundCounter[sourceSpriteId] = st_roundCounter[sourceSpriteId] + 1
	end
	
	local roundTime = isHasted and 2400 or 4800
	if Infinity_GetGameTicks() - st_roundTimer[sourceSpriteId] >= roundTime then
		st_roundCounter[sourceSpriteId] = 0
	end
	st_roundTimer[sourceSpriteId] = Infinity_GetGameTicks()
	...
	if isHasted then
		...
		local halfRoundIndex = st_roundCounter[sourceSpriteId] % 2
		local fullRoundIndex = math.floor(st_roundCounter[sourceSpriteId] / 2)
